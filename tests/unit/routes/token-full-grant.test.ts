...
describe("POST /api/token — Full grant flow", () => {
    ...
    beforeEach(async () => {
        ...
        await registerAgentToken(BASE_ENV.KV, "flow-agent-token", "test-agent", "Flow Test Agent");
    });
    ...
    it("returns 200 with token when consent exists", async () => {
        ...
        mockFetch
            .mockResolvedValueOnce(jsonResponse({ id: 12345, account: { login: "owner" } }))
            .mockResolvedValueOnce(jsonResponse({ token: "admin_token_123", expires_at: "2026-12-31T23:59:59Z", permissions: { administration: "write" }, repository_selection: "selected" }))
            .mockResolvedValueOnce(jsonResponse({ name: "repo" }))
            .mockResolvedValueOnce(jsonResponse({ token: "ghs_scoped_token_456", expires_at: "2027-01-01T00:00:00Z", permissions: { contents: "read" }, repository_selection: "selected" }));
    ...
    it("returns 200 with token for repo with admin scope", async () => {
        ...
        mockFetch
            .mockResolvedValueOnce(jsonResponse({ id: 999, account: { login: "admin" } }))
            .mockResolvedValueOnce(jsonResponse({ token: "admin_token_999", expires_at: "2026-12-31T23:59:59Z", permissions: { administration: "write" }, repository_selection: "selected" }))
            .mockResolvedValueOnce(jsonResponse({ name: "repo" }))
            .mockResolvedValueOnce(jsonResponse({ token: "ghs_admin_scoped", expires_at: "2027-06-01T00:00:00Z", permissions: { contents: "write", workflows: "write", administration: "write" }, repository_selection: "selected" }));
    ...
    it("creates repo and issues token when repo does not exist", async () => {
        const tokenService = new TokenService(env.KV);
        await tokenService.recordConsent("test-agent", "neworg/new-repo", ["contents:write"]);
        mockFetch
            .mockResolvedValueOnce(jsonResponse({ id: 77, account: { login: "neworg" } }))
            .mockResolvedValueOnce(jsonResponse({ token: "admin_create_token", expires_at: "2026-12-31T23:59:59Z", permissions: { administration: "write" }, repository_selection: "selected" }))
            .mockResolvedValueOnce(jsonResponse({ message: "Not found" }, 404))
            .mockResolvedValueOnce(jsonResponse({ login: "neworg" }))
            .mockResolvedValueOnce(jsonResponse({ name: "new-repo", full_name: "neworg/new-repo" }, 201))
            .mockResolvedValueOnce(jsonResponse({ token: "ghs_created_repo_token", expires_at: "2027-03-01T00:00:00Z", permissions: { contents: "write" }, repository_selection: "selected" }));
    ...
    it("proceeds normally when rate limiter is not configured", async () => {
        const envWithoutRl: HonoEnv["Bindings"] = { ...makeEnv(pkcs8Pem), TOKEN_RATE_LIMITER: undefined as unknown as RateLimit };
        const tokenService = new TokenService(env.KV);
        await tokenService.recordConsent("test-agent", "norl/repo", ["contents:read"]);
        mockFetch
            .mockResolvedValueOnce(jsonResponse({ id: 55, account: { login: "norl" } }))
            ...
            .mockResolvedValueOnce(jsonResponse({ name: "repo" }))
            .mockResolvedValueOnce(jsonResponse({ token: "ghs_rl_disabled", expires_at: "2027-01-01T00:00:00Z", permissions: { contents: "read" }, repository_selection: "selected" }));
    ...
    it("sanitizes error responses — does not leak GitHub API body details", async () => {
        const tokenService = new TokenService(env.KV);
        await tokenService.recordConsent("test-agent", "leaky/repo", ["contents:read"]);
        mockFetch.mockResolvedValueOnce(jsonResponse({ message: "Sensitive internal error details" }, 500));
        const client = testClient(app, makeEnv(pkcs8Pem));
        const resp = await client.api.token.$post(
    ...
    it("returns generic error message for non-matching error patterns", async () => {
        ...
        const tokenService = new TokenService(env.KV);
        await tokenService.recordConsent("test-agent", "weird/repo", ["contents:read"]);

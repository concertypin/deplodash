    ...
    it("revokes consent and redirects to /", async () => {
        ...
        const resp = await app.fetch(
            new Request("http://localhost/auth/revoke", {
                ...
                body: new URLSearchParams({ repo: "owner/repo", scopes: "contents:read", agent_id: "test-agent" }),
            }), authEnv
        );
        expect(resp.status).toBe(302);
        expect(resp.headers.get("Location")).toBe("/");
        expect(await tokenService.checkConsent("test-agent", "owner/repo", ["contents:read"])).toBe(false);
    });

    it("redirects with error when revoking fails", async () => {
        vi.spyOn(TokenService.prototype, "revokeConsent").mockRejectedValue(new Error("KV delete failed"));
        const authEnv: HonoEnv["Bindings"] = { ...BASE_ENV, GITHUB_TOKEN: "ghp_test_user_token" };
        const app = new Hono<HonoEnv>().use("*", sessionMiddleware()).route("/auth", consentRouter);
        ...
        expect(resp.status).toBe(302);
        expect(resp.headers.get("Location")).toContain("error=Failed+to+revoke+consent");
        vi.restoreAllMocks();
    });
});

...
describe("TokenService — revoke", () => {
    ...
    describe("revokeAllConsentsForRepo", () => {
        it("revokes all consents for a given repo", async () => {
            ...
            expect(
                await service.checkConsent("test-agent", "target/repo", [
                    "contents:read",
                ])
            ).toBe(false);
            expect(
            ...
            expect(
                await service.checkConsent("test-agent", "other/repo", [
                    "contents:read",
                ])
            ).toBe(true);
        });
        ...
        it("revokes consents from all agents when agentId is not provided", async () => {
            ...
            expect(
                await service.checkConsent("agent-1", "shared/repo", [
                    "contents:read",
                ])
            ).toBe(false);
            expect(
                await service.checkConsent("agent-2", "shared/repo", [
                    "contents:write",
                ])
            ).toBe(false);
            expect(
                await service.checkConsent("agent-1", "other/repo", [
                    "contents:read",
                ])
            ).toBe(true);
        });
    ...
    describe("revokeConsent with caller check", () => {
        it("succeeds when caller matches granted_by", async () => {
            ...
            expect(
                await service.checkConsent("agent-a", "alpha/repo", [
                    "contents:read",
                ])
            ).toBe(false);
        });

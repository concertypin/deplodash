...
describe("TokenService — cache & requestToken", () => {
    ...
    describe("token caching", () => {
        it("returns null when no cached token exists", async () => {
            const result = await service.getCachedToken("test-agent", "owner/repo", ["contents:read"]);
            expect(result).toBeNull();
        });

        it("returns cached token after caching", async () => {
            const future = new Date(Date.now() + 60 * 60 * 1000).toISOString();
            await service.cacheToken("test-agent", "owner/repo", ["contents:read"], "ghs_test", future);
            const cached = await service.getCachedToken("test-agent", "owner/repo", ["contents:read"]);
        ...
        it("returns null for expired cached token", async () => {
            ...
            await service.cacheToken("test-agent", "owner/repo", ["contents:read"], "ghs_test", past);
            const cached = await service.getCachedToken("test-agent", "owner/repo", ["contents:read"]);
        ...
        it("does not cache token that is too close to expiry", async () => {
            ...
            await service.cacheToken("test-agent", "owner/repo", ["contents:read"], "ghs_near_expiry", nearExpiry);
            const cached = await service.getCachedToken("test-agent", "owner/repo", ["contents:read"]);
            expect(cached).toBeNull();
        });
        ...
        it("caches token with 1 hour max TTL", async () => {
            ...
            await service.cacheToken("test-agent", "owner/repo", ["contents:read"], "ghs_long", farFuture);
            const cached = await service.getCachedToken("test-agent", "owner/repo", ["contents:read"]);
            expect(cached).not.toBeNull();
    ...
    describe("requestToken", () => {
        it("returns needs_consent when no consent exists and no cache", async () => {
            const result = await service.requestToken(
                { repo: "owner/repo", scopes: ["contents:read"], baseUrl: "http://test", agentId: "test-agent" },
        ...
        it("returns cached token on subsequent calls", async () => {
            ...
            const getToken = () => {
                callCount++;
                return Promise.resolve({ token: `ghs_${callCount}`, expires_at: new Date(Date.now() + 3600000).toISOString() });
            };

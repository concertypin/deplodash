...
describe("Session-Based Authorization Boundaries", () => {
    it("authGuard renders login page when no token is available", async () => {
        ...
        const app = new Hono<HonoEnv>()
            .use("*", (async (c, next) => authGuard()(c, next)) satisfies MiddlewareHandler<HonoEnv>)
            .get("/protected", (c) => c.json({ secret: "data" }));

        const resp = await app.fetch(new Request("http://localhost/protected"), BASE_ENV);
    ...
    it("authGuard uses GITHUB_TOKEN as fallback (dev bypass)", async () => {
        const envWithToken: HonoEnv["Bindings"] = { ...BASE_ENV, GITHUB_TOKEN: "ghp_dev_bypass_token" };
        const { authGuard } = await import("@/middleware");
        const app = new Hono<HonoEnv>().use("*", authGuard()).get("/protected", (c) => c.json({ status: "ok" }));

        const resp = await app.fetch(new Request("http://localhost/protected"), envWithToken);
        expect(resp.status).toBe(200);
    ...
    it("sessionMiddleware does not set gh_token with tampered cookie", async () => {
        ...
        const resp = await app.fetch(
            new Request("http://localhost/check", { headers: { Cookie: "session=tampered.invalid.value" } }),
            BASE_ENV
        );
        expect(resp.status).toBe(200);
        const body = await resp.json();
    ...
    it("sessionMiddleware does not set gh_token with expired session", async () => {
        ...
        const encrypted = await encryptWith(key, JSON.stringify(expiredPayload));
        ...
        const resp = await app.fetch(
            new Request("http://localhost/check", { headers: { Cookie: cookie } }),
            BASE_ENV
        );
        expect(resp.status).toBe(200);
        const body = await resp.json();
